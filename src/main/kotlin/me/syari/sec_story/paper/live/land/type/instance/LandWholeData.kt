...
class LandWholeData(
    ...
    override val type = LandType.Land
    ...
    override val adminInventory = { player: Player ->
        inventory("&9&l運営設定", 1) {
            id = "Land-Content-${this@LandWholeData.id}-Admin"
            ...
            if (particleTask != null) {
                ...
                item(3, Material.LIME_DYE, "&c&l枠を表示する")
                    .event(ClickType.LEFT) {
                        ...
                        particleTask = particle.spawnRepeatTimes(1, 0.1, plugin, 10, 240)?.onEndRepeat {
                            particleTask = null
                        }
                        reopen(id) {
                            openInventory(it)
                ...
                .event(ClickType.LEFT) {
                    inventory("&0&l登録解除確認", 1){
                        item(4, Material.LAVA_BUCKET, "&c&l本当に解除しますか？")
                            .event(ClickType.LEFT){
                                closeStartsWith("Land-Content-${this@LandWholeData.id}")
                                deleteContent()
                            }
    ...
    private fun CustomInventory.setDisplayArea(index: Int, player: Player){
        item(index, Material.FIREWORK_ROCKET, "&c&l範囲を表示する")
            .event(ClickType.LEFT){
                ...
                val particle = cubeOutLine(ParticleElement.Normal(Particle.FLAME), region)
                close(player)
                reopenStartsWith("Land-Content-${this@LandWholeData.id}"){
                    openInventory(it)
                }
                particleTask = particle.spawnRepeatTimes(1, 0.1, plugin, 10, 20)?.onEndRepeat {
                    particleTask = null
                }
            }
    }
    ...
    override val buyInventory = { player: Player ->
        inventory("&9&l土地購入", 1){
            id = "Land-Content-${this@LandWholeData.id}-Buy"
    ...
    override val ownerInventory = { player: Player ->
        inventory("&9&l土地設定", 1){
            id = "Land-Content-${this@LandWholeData.id}-Owner"
